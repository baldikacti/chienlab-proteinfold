...
class TSV2AFConverter:
    def __init__(self, workdir: str = ".") -> None:
        ...
        self.sequence_cache: dict[str, str | None] = {}
    ...
    def fetch_uniprot_sequences_batch(
        ...
        logger.info(f"Fetching {len(uncached_ids)} UniProt sequences in batches...")
    ...
    def _fetch_batch(self, uniprot_ids: list[str]) -> dict[str, str | None]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    for uniprot_id in uniprot_ids:
                        ...
                        if results[uniprot_id] is None:
                            logger.warning(
                                f"UniProt sequence not found for {uniprot_id}"
                            )
                ...
                logger.error(f"Request error in batch (attempt {attempt + 1}): {e}")
    ...
    def prefetch_uniprot_sequences(self, df: pd.DataFrame) -> None:
        ...
        if uniprot_ids:
            logger.info(f"Pre-fetching {len(uniprot_ids)} unique UniProt sequences...")
            self.fetch_uniprot_sequences_batch(uniprot_ids)
            logger.info("Pre-fetching complete!")

    def read_fasta(self, fasta_file: str | Path) -> str | dict[str, str]:
    ...
    def get_protein_sequence(self, entry: str) -> str | dict[str, str]:
            ...
            if isinstance(sequences, dict):
                ...
                for header, seq in sequences.items():
                    if self.is_dna_sequence(seq):
                        raise ValueError(
                            f"FASTA file {entry} entry '{header}' contains DNA sequence, not protein"
                        )
                    if self.is_rna_sequence(seq):
                        raise ValueError(
                            f"FASTA file {entry} entry '{header}' contains RNA sequence, not protein"
                        )
                return sequences
            else:
        ...
        if entry_type == "ccd_code":
            ...
            try:
                ...
                if isinstance(sequences, str):
                    # Single sequence
                    if self.is_dna_sequence(sequences):
                        return {"dna": {"id": sequence_id, "sequence": sequences}}
                    elif self.is_rna_sequence(sequences):
                        return {"rna": {"id": sequence_id, "sequence": sequences}}
                    else:
                        return {"protein": {"id": sequence_id, "sequence": sequences}}
                else:
                    ...
                    for i, (header, seq) in enumerate(sequences.items()):
                        ...
                        if self.is_dna_sequence(seq):
                            sequence_objects.append(
                                {"dna": {"id": current_id, "sequence": seq}}
                            )
                        elif self.is_rna_sequence(seq):
                            sequence_objects.append(
                                {"rna": {"id": current_id, "sequence": seq}}
                            )
                        else:
                            sequence_objects.append(
                                {"protein": {"id": current_id, "sequence": seq}}
                            )
                    return {
                        "multiple": sequence_objects
                    }  # Special marker for multiple sequences
            ...
            sequence = self.fetch_uniprot_sequence(entry)
            if sequence:
                return {"protein": {"id": sequence_id, "sequence": sequence}}
            else:
    ...
    def create_json_for_combination(
        self, bait_entry: str, prey_entry: str, output_dir: str | Path
    ) -> list[Path]:
        ...
        if not bait_sequences or not prey_sequences:
            raise RuntimeError(
                f"No valid sequences for combination {bait_entry}-{prey_entry}"
            )
    ...
    def create_fasta_for_colab_combination(
        self, bait_entry: str, prey_entry: str, output_dir: str | Path
    ) -> list[Path]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Error creating FASTA file for combination {bait_entry}-{prey_entry}: {e}"
            )

    def create_fasta_for_boltz_combination(
        self, bait_entry: str, prey_entry: str, output_dir: str | Path
    ) -> list[Path]:
    ...
    def convert(
        self, tsv_file: str | Path, output_dir: str = "output", mode: str = "alphafold3"
    ) -> list[Path]:
        ...
        if mode == "colabfold":
            ...
            if invalid_entries:
                raise ValueError(
                    f"ColabFold mode only supports UniProt IDs and FASTA files with protein sequences. "
        ...
        logger.info(f"Found {len(combinations)} bait-prey combinations")
        ...
        for i, (bait, prey) in enumerate(combinations, 1):
            if mode == "alphafold3":
                ...
                filepaths = self.create_fasta_for_colab_combination(
                    bait, prey, output_dir
                )
                created_files.extend(filepaths)
            elif mode == "boltz":
                filepaths = self.create_fasta_for_boltz_combination(
                    bait, prey, output_dir
                )
                created_files.extend(filepaths)
            else:
                raise ValueError(
                    f"Unknown mode '{mode}'. Supported modes: alphafold3, colabfold, boltz"
                )
        ...
        logger.info(
            f"Completed! Created {len(created_files)} {file_type} files in '{output_dir}' directory"
        )
        return created_files
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Convert TSV to AlphaFold3 JSON format or ColabFold FASTA format"
    )
    ...
    parser.add_argument(
        "--workdir", default=".", help="Work directory for relative paths (default: .)"
    )
    parser.add_argument(
        "--mode",
        choices=["alphafold3", "colabfold", "boltz"],
        default="alphafold3",
    ...
    if not Path(args.input_tsv).exists():
        logger.error(f"Error: Input file {args.input_tsv} does not exist")
        sys.exit(1)
